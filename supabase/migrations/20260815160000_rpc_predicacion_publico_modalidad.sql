-- Expone `modalidad` y `tipo_salida` en el RPC público de predicación.
-- La vista compartida (y la tarjeta de Inicio en modo público) arman el
-- programa desde este RPC, que lista los campos uno por uno; sin estos dos
-- una salida por grupo individual con cartas/teléfono se veía sólo con la
-- hora, sin poder describir de qué salida se trataba.
-- Definición copiada de 20260604221342 con esos dos campos agregados.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'programa', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'modalidad', pp.modalidad,
          'tipo_salida', pp.tipo_salida,
    ...
    -- Participantes referenciados como capitanes (en capitan_id o dentro de asignaciones_grupos)
    'participantes_capitanes', COALESCE((
      ...
      WHERE p.congregacion_id = _congregacion_id
        AND p.id IN (
          SELECT pp.capitan_id
          FROM public.programa_predicacion pp
          WHERE pp.congregacion_id = _congregacion_id
            AND pp.activo = true
            AND pp.fecha >= _desde AND pp.fecha <= _hasta
            AND pp.capitan_id IS NOT NULL
          UNION
          SELECT (a->>'capitan_id')::uuid
          FROM public.programa_predicacion pp,
               jsonb_array_elements(COALESCE(pp.asignaciones_grupos, '[]'::jsonb)) a
          WHERE pp.congregacion_id = _congregacion_id
            AND pp.activo = true
            AND pp.fecha >= _desde AND pp.fecha <= _hasta
            AND (a->>'capitan_id') IS NOT NULL
        )
    ), '[]'::jsonb)
  ) INTO _result;
